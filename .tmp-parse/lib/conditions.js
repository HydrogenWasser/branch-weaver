export const BOOLEAN_CONDITION_OPERATORS = ["eq", "neq"];
export const NUMBER_CONDITION_OPERATORS = ["eq", "neq", "gt", "gte", "lt", "lte"];
export function getAllowedOperators(valueType) {
    return valueType === "boolean" ? BOOLEAN_CONDITION_OPERATORS : NUMBER_CONDITION_OPERATORS;
}
export function isOperatorAllowed(valueType, operator) {
    return getAllowedOperators(valueType).includes(operator);
}
export function getDefaultValueForGlobalType(valueType) {
    return valueType === "boolean" ? false : 0;
}
export function coerceConditionValue(valueType, value) {
    if (valueType === "boolean") {
        return value === true;
    }
    return typeof value === "number" && Number.isFinite(value) ? value : 0;
}
export function createDefaultCondition(storyGlobal) {
    return {
        type: "atomic",
...
export function createCompositeCondition(type) {
...
export function evaluateCondition(condition, globalsById, values) {
    ...
    if (condition.type === "all") {
        return condition.conditions.every((child) => evaluateCondition(child, globalsById, values));
    }
    return condition.conditions.some((child) => evaluateCondition(child, globalsById, values));
}
export function isChoiceVisible(choice, globalsById, values) {
    return choice.visibilityCondition
        ? evaluateCondition(choice.visibilityCondition, globalsById, values)
        : true;
}
export function resolveChoiceTargetNodeId(choice, globalsById, values) {
    if (choice.route.mode === "direct") {
        return choice.route.targetNodeId;
    }
    const matchedBranch = choice.route.branches.find((branch) => evaluateCondition(branch.condition, globalsById, values));
    return matchedBranch?.targetNodeId ?? choice.route.fallbackTargetNodeId;
}
export function getChoiceRouteTargets(route) {
    if (route.mode === "direct") {
        return route.targetNodeId ? [route.targetNodeId] : [];
    }
    return [
        ...route.branches.map((branch) => branch.targetNodeId).filter((targetNodeId) => Boolean(targetNodeId)),
        ...(route.fallbackTargetNodeId ? [route.fallbackTargetNodeId] : [])
    ];
}
export function getChoiceReferencedGlobalIds(choice) {
    const ids = new Set();
...
export function applyEffects(effects, globalsById, values) {
...
export function replaceRouteTargetNodeId(route, removedNodeId) {
    if (route.mode === "direct") {
        return {
            ...route,
    ...
    return {
        ...
        fallbackTargetNodeId: route.fallbackTargetNodeId === removedNodeId ? null : route.fallbackTargetNodeId
    };
}
export function formatOperatorLabel(operator) {
    switch (operator) {
        case "eq":
            return "=";
        case "neq":
...
export function formatConditionSummary(condition, globalsById) {
    if (condition.type === "atomic") {
        const storyGlobal = globalsById.get(condition.globalId);
...
export function formatEffectsSummary(choice, globalsById) {
...
export function formatChoiceSummary(choice, globalsById) {
    const parts = [];
    if (choice.visibilityCondition) {
